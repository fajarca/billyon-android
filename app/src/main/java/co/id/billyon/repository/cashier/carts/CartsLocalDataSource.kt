...
class CartsLocalDataSource @Inject constructor(private val dao: CartsDao) {

    fun insert(cart : Carts) = dao.insert(cart)
    fun delete(cart : Carts) = dao.delete(cart)
    fun update(cart: Carts) = dao.update(cart)
    fun updateQuantity(productId: Long, quantity : Int) = dao.updateQuantity(productId, quantity)
